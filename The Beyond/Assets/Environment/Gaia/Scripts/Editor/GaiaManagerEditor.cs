...
namespace Gaia
{
    ...
    public class GaiaManagerEditor : EditorWindow
    {
        ...
        private string[] m_menuStrings2 = new string[] { "More on Gaia", "More from Procedural Worlds" };
...
//                GUILayout.EndVertical();
        ...
        void DrawAdvancedEditor()
        {
            ...
            if (m_foldoutSpawners = EditorGUILayout.Foldout(m_foldoutSpawners, GetLabel("3. Create and configure your Spawners...")))
            {
                ...
                if (DisplayButton(GetLabel("Create Clustered Tree Spawner From Terrain")))
                {
                    Selection.activeObject = CreateClusteredTreeSpawnerFromTerrainTrees();
                }
                if (DisplayButton(GetLabel("Create Clustered Tree Spawner From GameObjects")))
                {
                ...
                if (DisplayButton(GetLabel("Create Coverage Tree Spawner From GameObjects")))
                {
            ...
            if (m_foldoutUtils = EditorGUILayout.Foldout(m_foldoutUtils, GetLabel("5. Handy Utilities...")))
            {
                EditorGUI.indentLevel++;
...
//            GUILayout.Space(2);
        ...
        void DrawMoreOnGaiaEditor()
        {
            EditorGUI.indentLevel++;
            DrawBodyText("Improve your skills with our tutorials and documentation.");
            GUILayout.Space(5f);

            if (DrawLinkHeaderText(GetLabel("Gaia Documentation & Tutorials")))
            {
                Application.OpenURL("http://www.procedural-worlds.com/gaia/?section=tutorials");
            }
            DrawBodyText("You can access the Gaia QuickStart guide and other product documentation in the Gaia / Documentation directory, or click on the heading link to view Gaia tutorials.");
            GUILayout.Space(5f);
            ...
            DrawBodyText("Gaia eXtensions accelerate and simplify development. Check out the other amazing assets we have integrated for you in the link above. If you are a publisher and would like an integration then please contact us via our support system.");
            GUILayout.Space(5f);
            ...
            DrawBodyText("Click on the heading link to lodge a support request. We do support on week days and aim to answer all queries within 48 hours.");
            GUILayout.Space(5f);

            if (DrawLinkHeaderText(GetLabel("Help Us To Help You - Leave a Review!")))
            {
                Application.OpenURL("https://www.assetstore.unity3d.com/#!/content/42618?aid=1101lSqC");
            }
            DrawBodyText("It takes time, dedication, and an active community to thrive and grow! Please us to grow by spreading the word and leaving a review on the asset store.");
            GUILayout.Space(5f);
        ...
        {
            EditorGUI.indentLevel++;
            DrawBodyText("More from Procedural Worlds.");
            GUILayout.Space(5f);
            ...
            if (DrawLinkHeaderText("Our Community"))
            {
                Application.OpenURL("https://discord.gg/rtKn8rw");
            }
            DrawBodyText("Got a question? Want to see some cool community art? Or perhaps you just feel like chatting with people who share your love of game development. Click on the link and head over to our Discord server!");
            GUILayout.Space(5f);
            ...
            DrawBodyText("While Gaia is at its heart a terrain and scene creation system, it is the generous support of our partners and their awesome content samples that makes Gaia such a great starting point for your game. Click on the heading to see more of their amazing work!");
            GUILayout.Space(5f);
...
//            Handles.EndGUI();
        ...
        static Dictionary<string, string> m_tooltips = new Dictionary<string, string>
        {
            ...
            { "Show Scanner", "Shows the scanner. Use the scanner to create new stamps from textures, world machine .r16 files, IBM 16 bit RAW files, Terrains, and Meshes (with mesh colliders)." },
